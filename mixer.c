...
Mix_Chunk *Mix_LoadWAV_RW(SDL_RWops *src, int freesrc)
{
    ...
    switch (magic) {
        case WAVE:
        case RIFF:
			loaded = Mix_LoadWAV_RW_(src, freesrc, &wavespec,
	...
	/* MODIFICATION: if unsupported freq, convert it in advance */
	if (wavespec.freq != 44100 &&
		wavespec.freq != 22050 &&
		wavespec.freq != 11025) {
		AudioFreqConvert(chunk, wavespec.channels, wavespec.format, wavespec.freq, mixer.freq);
